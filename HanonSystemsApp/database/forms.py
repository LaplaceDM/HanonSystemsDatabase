...
class Test_HarnessForm(ModelForm):
    date_inserted = forms.DateField(widget=forms.DateInput(attrs={"class": "form-control", "type": "date"}))
...
class TestUpdateForm(ModelForm):
    ...
    choices2 = (
        ...
        ("1-setup", "1-setup"),
        ("2-running", "2-running"),
        ("3-data review", "3-data review"),
        ("4-on hold", "4-on hold"),
        ("5-no man power", "5-no man power"),
        ("6-on track", "6-on track"),
    )
    choices3 = (
        (None, "-----"),
        (1, 1),
    ...
    supervisor_comments = forms.CharField(widget=forms.Textarea(attrs={"rows": "3"}), required=False)
    scheduling = forms.ChoiceField(widget=forms.Select, choices=choices1, required=False)
    status = forms.ChoiceField(widget=forms.Select, choices=choices2, required=False)
    priority = forms.ChoiceField(widget=forms.Select, choices=choices3, required=False)

    program_id = forms.ModelChoiceField(queryset=Program.objects.all(), required=False, label="Program")

    test_map_id = forms.ModelChoiceField(queryset=TestMap.objects.all(), required=False, label="Test Map")

    product_id = forms.ModelChoiceField(queryset=Product.objects.all(), required=False, label="Product")

    test_type_id = forms.ModelChoiceField(queryset=TestType.objects.all(), required=False, label="Test Type")

    test_description = forms.CharField(required=False, label="Test Description")

    technician_id = forms.ModelChoiceField(queryset=Technician.objects.all(), required=False, label="Technician")

    chamber_id = forms.ModelChoiceField(queryset=Chamber.objects.all(), required=False, label="Chamber")

    dar_id = forms.ModelChoiceField(queryset=DAR.objects.all(), required=False, label="DAR")

    cage_id = forms.ModelChoiceField(queryset=Cage.objects.all(), required=False, label="Cage")

    lab_id = forms.ModelChoiceField(queryset=Lab.objects.all(), required=False, label="Lab")

    hours_planned = forms.IntegerField(required=False, label="Hours Planned")

    total_hours = forms.IntegerField(required=False, label="Total Hours")
...
class DUTForm(ModelForm):
    received_date = forms.DateField(widget=forms.DateInput(attrs={"class": "form-control", "type": "date"}))
    storage_date = forms.DateField(widget=forms.DateInput(attrs={"class": "form-control", "type": "date"}))
