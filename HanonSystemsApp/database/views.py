...
class UpdateTableViewTest(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if test.supervisor_comments:
            if (
                test.supervisor_comments in instance.supervisor_comments
                and test.supervisor_comments != instance.supervisor_comments
            ):
...
def clone_item(request, pk):
    if request.user.has_perm("database.change_test"):
        ...
        ch = ChamberLogInfo(
            ...
            test_id=Test.objects.get(pk=obj.pk),
...
def chamber_schedule(request):
    ...
    try:
        ...
        chamber_tests = (
            Test.objects.filter(Q(scheduling="current") | Q(scheduling="upcoming"))
        ...
        for i in range(len(chamber_tests)):
            a.write(
                f'"booking{i}": {{"targeted_start" : "{chamber_tests[i].targeted_start}", "targeted_end" : "{chamber_tests[i].targeted_end}",'
...
def dar_schedule(request):
    ...
    try:
        ...
        for i in range(len(dar_tests)):
            a.write(
                f'"booking{i}": {{"targeted_start" : "{dar_tests[i].targeted_start}", "targeted_end" : "{dar_tests[i].targeted_end}",'
...
def cage_schedule(request):
    ...
    try:
        ...
        for i in range(len(cage_tests)):
            a.write(
                f'"booking{i}": {{"targeted_start" : "{cage_tests[i].targeted_start}", "targeted_end" : "{cage_tests[i].targeted_end}",'
...
def calculate(request):
    ...
    all_logs = ChamberLog.objects.filter(timestamp__gte=dates[0]).filter(
        timestamp__lte=dates[1]
    )  # getting all logs in the specified time period
    ...
    for chamber in chambers:
        ...
        for test in chamber_logs:
            ...
            test_logs = chamber_logs.filter(log_id__test_id=test.log_id.test_id).order_by(
                "timestamp"
            )  # getting chamber logs that belong to current test
            ...
            try:  # checking for log of test before the selected time period
                previous_log = (
                    ChamberLog.objects.filter(log_id__test_id=logs[0].log_id.test_id)
                    .filter(chamber_id=logs[0].chamber_id)
                ...
                for log in logs:
                    if log == logs[0]:  # in case this is the leading log, check if the previous log's present
                        if previous_log == 0:  # case 1:first log of test
                            ...
                            try:
                                last_log = (
                                    ChamberLog.objects.filter(log_id__test_id=log.log_id.test_id)
                                    .filter(chamber_id=log.chamber_id)
                                    .filter(circuit_number=log.circuit_number)
                                    .filter(timestamp__lt=log.timestamp)
                                    .latest("timestamp")
                                )
                            except:
                                first_log = True  # case 2:no previous log in current circuit, but there is a previous log from another circuit
                                switch_date = (
                                    str(previous_log.timestamp.day)
                                    + "/"
                                    + str(previous_log.timestamp.month)
                                    + "/"
                                    + str(previous_log.timestamp.year)
                                )
                                ...
                                in_between = (log.timestamp - time_adjustment).days * max_hours + (
                                    log.timestamp - time_adjustment
                                ).seconds / 60 / 60

                                if (
                                    in_between >= log.total_hours
                                ):  # case 2.1: if running hours recorded overlaps the hours up until the previous log
                                    overlap = False
                                else:
                                    overlap = True
                                ...
                                ):  # checking to see if last log is before or after where we switched, if this is the first log of the time period, "try" makes sure that value 0 doesn't cause errors
                    ...
                    if first_log == True:  # Case 1 and 2's:
                        ...
                        if (
                            program_name not in program_hours[chamber_name].keys()
                        ):  # Step 2: updating program hours, for logs that lead a series of logs
                            ...
                            program_hours[chamber_name][program_name]["stopped"] = (
                                program_hours[chamber_name][program_name]["stopped"] + status_hours
                            )
                        ...
                        if (
                            log.status == "stopped" or log == logs[len(logs) - 1]
                        ):  # when the current log is the last of the circuit logs, or the stretch of logs is stopped.
                            try:
                                ...
                                logs = (
                                    test_logs.filter(timestamp__gt=log.timestamp)
                                    .filter(circuit_number=next_log.circuit_number)
                                    .order_by("timestamp")
                                )
                                break
                        continue
                    ...
                    # Case 3's:
                    running_hours = (
                        log.total_hours - previous_log.total_hours
                    )  # Step 1: calculating hours between two subsequent logs
                    ...
                    previous_date = (
                        str(previous_timestamp.day)
                        + "/"
                        + str(previous_timestamp.month)
                        + "/"
                        + str(previous_timestamp.year)
                    )

                    if date == previous_date:  # Case 1: when two subsequent logs are on the same day
                        in_between = (
                            (current_timestamp - previous_timestamp).seconds / 60 / 60
                        )  # only time to be accounted for is that between two logs
                        try:
                        ...
                        if (
                            status_hours < 0
                        ):  # in case the assumption of max daily hours is incorrect, this prevents subtractions from previously summed status hours
                            status_hours = 0
                        ...
                        endofday_previous = datetime(
                            previous_timestamp.year, previous_timestamp.month, previous_timestamp.day, 23, 59, 59, 0
                        )
                        startofday_current = datetime(
                            current_timestamp.year, current_timestamp.month, current_timestamp.day, 0, 0, 0, 0
                        )
                        ...
                        )  # adding duration of in between period
                        ...
                        if occupied_beginning + in_between < running_hours:  # updating used hours
                            if (
                                previous_log.status == "running"
                            ):  # Case 1: chamber run time can stretch from last log into the day of current log
                    ...
                    if program_name not in program_hours[chamber_name].keys():  # Step 2: updating program hours
                        ...
                        if previous_log.status not in program_hours[chamber_name][program_name].keys():
                            ...
                            if previous_log.status == "running":
                                ...
                                program_hours[chamber_name][program_name][previous_log.status] = (
                                    program_hours[chamber_name][program_name][previous_log.status] + status_hours
                                )
...
def dut_hours(request):
    ...
    for test in tests:
        total_hours = (
            ChamberLog.objects.filter(log_id__test_id=test.log_id.test_id)
            .filter(dut_id=dut)
            .latest("timestamp")
            .total_hours
        )
...
def harness_info(request, id):
    ...
    if test_list.count() == 0:
        return render(
            request,
            "html/harness_info.html",
            {"test_history": test_history, "harness_info": Harness.objects.filter(pk=id)},
        )
    ...
    for test in test_list:
        if test.date_inserted == None:
            ...
            try:
                starting_time = (
                    ChamberLog.objects.filter(log_id__test_id=test.test_id)
                    .filter(circuit_number=test.circuit_number)
                    .filter(timestamp__gte=test.date_inserted)
                    .earliest("timestamp")
                    .total_hours
                )
            except:
        ...
        if test.date_removed == None:
            ...
            try:
                ending_time = (
                    ChamberLog.objects.filter(log_id__test_id=test.test_id)
                    .filter(circuit_number=test.circuit_number)
                    .filter(timestamp__lte=test.date_inserted)
                    .latest("timestamp")
                    .total_hours
                )
            except:
        ...
        if test.test_id.test_type_id.test_name in test_history:
            test_history[test.test_id.test_type_id.test_name] = [
                total_hours + test_history[test.test_id.test_type_id.test_name][0],
                accumulated_hours,
                start_date,
                test.test_id.chamber_id.chamber_name,
            ]
        else:
            test_history[test.test_id.test_type_id.test_name] = [
                total_hours,
            ...
            if test.date_removed == None:
                ...
                try:
                    ending_time = (
                        ChamberLog.objects.filter(log_id__test_id=test.test_id)
                        .filter(circuit_number=test.circuit_number)
                        .filter(timestamp__lte=test.date_removed)
                        .latest("timestamp")
                        .total_hours
                    )
                except:
